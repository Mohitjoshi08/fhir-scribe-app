...
# AI Ambient Scribe â€” Mobile-First FHIR Clinical Notes

> **PS-1**: Mobile-First Ambient AI Scribe with Real-Time FHIR Conversion  

A **production-deployed**, mobile-first AI-powered clinical documentation tool that converts doctor-patient conversations into structured, **FHIR R4-compliant clinical data** in real-time. Supports **Hindi, English, and Hinglish** conversations with automatic English-only report generation and a full **Hindi/English UI toggle** â€” designed for Indian healthcare settings.

 **Live App**: Frontend on [Vercel](https://vercel.com) Â· Backend on [Render](https://render.com)  
 **Android APK**: Pre-built APK available (`AI-Ambient-Scribe.apk`) â€” install directly on any Android device

---

## Proposed Approach & Solution

Indian healthcare faces a critical documentation bottleneck â€” doctors in Tier 2/3 cities spend nearly 30â€“40% of their consultation time manually writing notes, often in a mix of Hindi and English (Hinglish). This unstructured, paper-based workflow makes it nearly impossible to generate interoperable health records that comply with modern standards like HL7 FHIR R4. Existing voice-to-text solutions predominantly support English and lack clinical context understanding, making them ineffective for the Indian healthcare landscape.

Our solution, **AI Ambient Scribe**, is a mobile-first web application that passively listens to doctor-patient conversations in real-time and automatically produces structured, FHIR R4-compliant clinical documentation â€” all from a single tap on the doctor's phone.

The system works in a three-stage pipeline:

1. **Audio Capture & Transcription** â€” The recorded audio (captured via the browser's MediaRecorder API as WebM) is sent to the FastAPI backend, where **Google Gemini 2.5 Flash** performs multilingual transcription with automatic speaker diarization, accurately tagging each line as "Doctor" or "Patient" â€” even in code-mixed Hinglish conversations.

2. **AI-Powered Clinical Extraction** â€” The transcript is processed by two parallel **Gemini 2.5 Flash Lite** pipelines: one generates structured clinical notes (Chief Complaint, HPI, Vitals, Diagnoses with ICD-10 codes, Medications with RxNorm codes, Follow-up, and Advice), while the other maps clinical entities to a full **FHIR R4 Bundle** containing Patient, Encounter, Observation, Condition, and MedicationRequest resources with proper SNOMED-CT, LOINC, and RxNorm coding. **All output is forced to English** regardless of conversation language, ensuring proper medicine database matching and readable clinical records.

3. **Medicine Enrichment** â€” Extracted medication names are matched against a **100MB Indian medicines dataset** (200K+ drugs) using fuzzy matching. Each prescription is enriched with standardized name, composition, manufacturer, therapeutic class, and price from the database.

4. **Validation & Save** â€” The generated bundle is automatically validated against the FHIR R4 schema, and results are displayed with a pass/fail badge. The doctor can then **fully edit** every field (including individual medication dosage, frequency, duration, route, and custom fields), **save reports** linked to patients in the hospital's **Supabase** database, and **download** a polished prescription PDF.
...
|  **Fully Editable Reports** | Edit every field inline â€” including individual medication name, dosage, frequency, duration, route, vitals, and diagnoses |
...
|  **Indian Medicine Database** | 200K+ medicines with fuzzy matching â€” enriches prescriptions with composition, manufacturer, therapeutic class |
|  **Pipeline Speed Metrics** | Transcription + FHIR processing time displayed in real-time |

### Admin Dashboard â€” Analytics & Monitoring
...
|  **Gemini API Usage Counter** | Real-time estimated API call counter (per report Ã— 3 calls) with daily/weekly breakdown and budget warnings |
| â± **Total Time Saved** | Complexity-based calculator scoring each report by vitals, diagnoses, medications, and text length |
|  **Doctor Activity Heatmap** | 4-week Ã— 7-day grid showing which days have the most consultations |
...
|  **Native Android APK** | Built with Capacitor â€” installable APK with native device features |
...
```
 Audio (WebM)
    â†’  Hinglish Transcript (verbatim, original language)
    â†’  Structured Notes (English only â€” auto-translated)
    â†’  Medicine Enrichment (Indian DB, 200K+ drugs)
    â†’  FHIR R4 Bundle (English, coded with SNOMED/ICD-10/LOINC/RxNorm)
    â†’  Validation
    â†’  Supabase
           â†• Doctor Edits â†•           â†• Auto-Sync â†•
```
...
| **Database** | Supabase (PostgreSQL â€” profiles, reports, shared reports) |
...
2. Enable **Email/Password** sign-in (Authentication â†’ Providers â†’ Email)
...
4. Note your **Project URL**, **Anon Key**, and **JWT Secret** from Project Settings â†’ API
...
| `POST` | `/api/transcribe/?language=hi-en` |  | Audio â†’ Text transcription (Gemini 2.5 Flash) |
| `POST` | `/api/fhir/` |  | Transcript â†’ FHIR Bundle + Structured Notes (English output) |
...
```
fhir-scribe-app/
â”œâ”€â”€ AI-Ambient-Scribe.apk       # Pre-built Android APK
â”œâ”€â”€ backend/
â”‚   â”œâ”€â”€ main.py                  # FastAPI app with CORS, logging, health endpoints
â”‚   â”œâ”€â”€ requirements.txt         # Python dependencies
â”‚   â”œâ”€â”€ render.yaml              # Render deployment config
â”‚   â”œâ”€â”€ .env                     # GEMINI_API_KEY + SUPABASE_URL + SUPABASE_JWT_SECRET
â”‚   â”œâ”€â”€ data/
â”‚   â”‚   â””â”€â”€ medicines.csv.gz     # Indian medicines database (200K+ drugs, gzipped)
â”‚   â””â”€â”€ services/
â”‚       â”œâ”€â”€ auth.py              # Supabase JWT verification (ES256 via JWKS)
â”‚       â”œâ”€â”€ transcription.py     # Audio â†’ text (Gemini 2.5 Flash + multilingual + diarization)
â”‚       â”œâ”€â”€ fhir_mapper.py       # Text â†’ FHIR R4 + structured notes (English-only output)
â”‚       â”œâ”€â”€ fhir_validator.py    # FHIR R4 validation engine
â”‚       â””â”€â”€ medicine_lookup.py   # Fuzzy medicine matching (exact â†’ prefix â†’ difflib)
â”œâ”€â”€ frontend/
â”‚   â”œâ”€â”€ index.html               # PWA-ready HTML
â”‚   â”œâ”€â”€ capacitor.config.ts      # Capacitor config for Android builds
â”‚   â”œâ”€â”€ vercel.json              # Vercel deployment config (SPA rewrites)
â”‚   â”œâ”€â”€ android/                 # Android Studio project (Capacitor)
â”‚   â”œâ”€â”€ src/
â”‚   â”‚   â”œâ”€â”€ supabase.ts          # Supabase client initialization
â”‚   â”‚   â”œâ”€â”€ i18n/
â”‚   â”‚   â”‚   â””â”€â”€ translations.ts  # English + Hindi translations (150+ strings)
â”‚   â”‚   â”œâ”€â”€ components/
â”‚   â”‚   â”‚   â””â”€â”€ LanguageToggle.tsx # Hindi/English toggle button
â”‚   â”‚   â”œâ”€â”€ contexts/
â”‚   â”‚   â”‚   â”œâ”€â”€ AuthContext.tsx   # Supabase Auth state, login/register/logout
â”‚   â”‚   â”‚   â””â”€â”€ LanguageContext.tsx # i18n language provider with localStorage
â”‚   â”‚   â”œâ”€â”€ hooks/
â”‚   â”‚   â”‚   â””â”€â”€ usePwaInstall.ts  # PWA install prompt hook
â”‚   â”‚   â”œâ”€â”€ utils/
â”‚   â”‚   â”‚   â””â”€â”€ pdfDownload.ts    # PDF generation (web + Capacitor native)
â”‚   â”‚   â”œâ”€â”€ pages/
â”‚   â”‚   â”‚   â”œâ”€â”€ LoginPage.tsx        # Doctor login + Hospital Patient Database access
â”‚   â”‚   â”‚   â”œâ”€â”€ RegisterPage.tsx     # Doctor registration + Hospital patient registration
â”‚   â”‚   â”‚   â”œâ”€â”€ DoctorDashboard.tsx  # Patient tracking, reports, share with other doctors
â”‚   â”‚   â”‚   â”œâ”€â”€ PatientDashboard.tsx # Patient medical records view
â”‚   â”‚   â”‚   â”œâ”€â”€ AdminDashboard.tsx   # Full analytics, charts, user management, monitoring
â”‚   â”‚   â”‚   â”œâ”€â”€ ScribePage.tsx       # Recording, FHIR pipeline, full editing, FHIR sync
â”‚   â”‚   â”‚   â”œâ”€â”€ ReportDetailPage.tsx # Full report detail view
â”‚   â”‚   â”‚   â””â”€â”€ PrintablePDFReport.tsx # A4 prescription PDF template
â”‚   â”‚   â”œâ”€â”€ App.tsx              # Router with protected & role-based routes
â”‚   â”‚   â”œâ”€â”€ App.css              # Custom animations, glassmorphism, chart styles
â”‚   â”‚   â”œâ”€â”€ index.css            # Tailwind + base styles
â”‚   â”‚   â””â”€â”€ main.tsx             # React entry with AuthProvider + LanguageProvider
â”‚   â”œâ”€â”€ package.json
â”‚   â”œâ”€â”€ vite.config.ts
â”‚   â””â”€â”€ tailwind.config.js
â”œâ”€â”€ Extensive_A_Z_medicines_dataset_of_India.csv  # Raw medicine dataset (100MB)
...
|-----------|---------------|--------|
| Capture conversations in real-time (Hindi + English mix) | MediaRecorder API â†’ Gemini 2.5 Flash transcription with Hinglish prompt & speaker diarization |  |
| Convert speech into structured clinical notes | Structured Notes extraction (CC, HPI, Vitals, Dx, Rx, Follow-up, Advice) â€” all fully editable with add/remove. English-only output. |  |
| Map entities to FHIR resources | Patient, Encounter, Observation, Condition, MedicationRequest with SNOMED/ICD-10/LOINC/RxNorm â€” auto-synced on edit |  |
...
> Update the Supabase **Site URL** and **Redirect URLs** in Authentication â†’ URL Configuration to your production domain.
